...
void CheckRatio(const Rational& rat, istream& is){
    if (is.fail()|| rat.Denominator() <= 0 || rat.Numerator() < 0){
        throw runtime_error("Invalid argument");
    }

}

void CheckChar(const Rational rat, char sign){
    if (sign = '/' && rat.Numerator() == 0){
...
int main(){
    try{
        ...
        char sign;
        cin >> r1 >> sign >> r2;
        CheckRatio(r1, cin); CheckRatio(r2, cin);
        CheckChar(r2, sign);
        if (sign == '+'){
            cout << r1 + r2 << endl;
        } else if (sign == '-'){
